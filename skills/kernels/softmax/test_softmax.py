"""Softmax correctness + performance test."""
...
def test_correctness(dtype=torch.float16):
    configs = [
        (32, 128), (64, 1024), (128, 4096), (256, 8192), (16, 50257),
    ]
    results = []
    for M, N in configs:
        ...
        ok = torch.allclose(out.float(), ref.float(), atol=1e-3, rtol=1e-3)
...
def benchmark(M=2048, N=4096, dtype=torch.float16, n_warmup=50, n_iter=200):
    ...
    total_bytes = M * N * elem_size * 2  # read + write
...
def main():
    ...
    print(f"=== Softmax Test on {gpu_name} ===\n")
